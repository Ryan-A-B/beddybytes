...
func (accountStore *AccountStoreInMemory) Put(account *Account) (err error) {
	existingAccount, ok := accountStore.accounts[account.AccountID]
	if ok {
...
func (accountStore *AccountStoreInMemory) create(account *Account) (err error) {
	err = accountStore.checkEmail(account.Email)
	if err != nil {
		return
	}
	accountStore.accounts[account.AccountID] = account
	accountStore.accountByEmail[account.Email] = account
	return
}
...
func (accountStore *AccountStoreInMemory) update(existingAccount *Account, account *Account) (err error) {
	if existingAccount.Email != account.Email {
		...
		delete(accountStore.accountByEmail, existingAccount.Email)
	}
	accountStore.accounts[account.AccountID] = account
	accountStore.accountByEmail[account.Email] = account
	return
...
func (accountStore *AccountStoreInMemory) Remove(accountID string) (err error) {
	...
	delete(accountStore.accountByEmail, account.Email)
	return
}
